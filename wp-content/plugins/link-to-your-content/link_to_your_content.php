	...
	if(!class_exists('LinkToYourContent')) {
		class LinkToYourContent {
			...
			function ltyc_network_menu() {
				if(function_exists('is_multisite') && is_multisite() && function_exists('is_plugin_active_for_network') && is_plugin_active_for_network($this->pluginFile)) {
					add_submenu_page( 'settings.php', 'Link to Your Content Options', 'Link to Your Content', 'manage_network', $this->optionsHook, array('LinkToYourContent','ltyc_options') );
				}
			}		
			...
			function ltyc_menu() {
				if(function_exists('is_multisite') && is_multisite() && function_exists('is_plugin_active_for_network') && is_plugin_active_for_network($this->pluginFile)) { return true; }
					...
					add_options_page('Link to Your Content Options', 'Link to Your Content', 'moderate_comments', $this->optionsHook, array('LinkToYourContent','ltyc_options'));
				}
			}
			...
			function ltyc_plugin_link($links, $file) {
				...
				if($file == $this_plugin) {
					$settings_link = '<a href="'.admin_url('options-general.php?page='.$this->optionsHook).'">Settings</a>';
			...
			function ltyc_network_plugin_link($links, $file) {
				if(function_exists('is_multisite') && is_multisite() && function_exists('is_plugin_active_for_network') && is_plugin_active_for_network($this->pluginFile)) {
					...
					if(empty($this_plugin)) $this_plugin = $this->pluginFile;
					if($file == $this_plugin) {
						$settings_link = '<a href="'.admin_url('network/settings.php?page='.$this->optionsHook).'">Settings</a>';
						array_unshift($links, $settings_link);
					}
					return $links;
				}
			...
			function ltyc_tinymce_plugin($plugin_array) {
				$plugin_array[$this->pluginName] = LTYCDLPLUGINPATH.'/editor_plugin.js';
				return $plugin_array;
			}
			...
			function ltyc_panel_display($type='posts') {
				...
				if($type != 'external') {
					...
					$panel_display .= $this->ltyc_document_list($type, $this->page, $this->pageOffset);
					$panel_display .= '</table>';
					$panel_display .= '</div>';
				}
				else {
					...
					$panel_display .= 'External URL: <input type="radio" name="externalLinkType" value="http" class="ltycRadio marginRight25" checked="checked" />';
					$panel_display .= 'E-mail Address: <input type="radio" name="externalLinkType" value="mailto" class="ltycRadio" />';
					$panel_display .= '</div>';
				}
				$panel_display .= '<table border="0" cellpadding="2" cellspacing="0">';
				$panel_display .= '<tr>';
				$panel_display .= '<td><label for="'.$type.'_linkTitle">Title:</label><br /><input type="text" id="'.$type.'_linkTitle" name="'.$type.'_linkTitle" value="" size="60" /></td>';
				$panel_display .= '</tr>';
				$panel_display .= '<tr>';
				$panel_display .= '<td><label for="'.$type.'_linkTarget">Target:</label><br />';
				$panel_display .= '<select name="'.$type.'_linkTarget" id="'.$type.'_linkTarget">';
				$panel_display .= '<option value="_self">Open in the same window</option>';
				$panel_display .= '<option value="_blank">Open in a new window</option>';
			...
			function ltyc_site_toggle() {
				...
				if(function_exists('is_multisite') && is_multisite() && isset($this->ltycOptions) && $this->ltycOptions['ltyc_multi_site'] == 1) {		
					...
					$siteToggle .= '<p><label for="siteToggle">Choose Site:</label> <select name="siteToggle" id="siteToggle" size="1">'."\n";
			...
			function ltyc_content_tabs() {
				...
				$contentTabs .= '<ul>'."\n";
				$contentTabs .= '<li id="posts_tab" class="current"><span><a href="javascript:mcTabs.displayTab(\'posts_tab\',\'posts_panel\');" onMouseDown="return false;">Posts</a></span></li>'."\n";
				$contentTabs .= '<li id="pages_tab"><span><a href="javascript:mcTabs.displayTab(\'pages_tab\',\'pages_panel\');" onMouseDown="return false;">Pages</a></span></li>'."\n";
				if(function_exists('get_post_types') && isset($this->ltycOptions) && $this->ltycOptions['ltyc_custom_post_type'] == 1) {
					$post_types = $wpdb->get_col($wpdb->prepare("SELECT DISTINCT(post_type) FROM ".$wpdb->prefix."posts WHERE post_type != 'post' AND post_type != 'page' AND post_type != 'revision' AND post_type != 'attachment' AND post_type != 'nav_menu_item' AND post_status = 'publish'"));	
					if($post_types) {
						$contentTabs .= '<li id="cptypes_tab"><span><a href="javascript:mcTabs.displayTab(\'cptypes_tab\',\'cptypes_panel\');" onMouseDown="return false;">Custom Post Types</a></span></li>'."\n";
					}
				}
				if(function_exists('get_taxonomies') && isset($this->ltycOptions) && $this->ltycOptions['ltyc_taxonomy'] == 1) {
					$contentTabs .= '<li id="taxonomies_tab"><span><a href="javascript:mcTabs.displayTab(\'taxonomies_tab\',\'taxonomies_panel\');" onMouseDown="return false;">Taxonomies</a></span></li>'."\n";
				}
				$contentTabs .= '<li id="docs_tab"><span><a href="javascript:mcTabs.displayTab(\'docs_tab\',\'docs_panel\');" onMouseDown="return false;">Docs</a></span></li>'."\n";
				$contentTabs .= '<li id="images_tab"><span><a href="javascript:mcTabs.displayTab(\'images_tab\',\'images_panel\');" onMouseDown="return false;">Images</a></span></li>'."\n";
				$contentTabs .= '<li id="external_tab"><span><a href="javascript:mcTabs.displayTab(\'external_tab\',\'external_panel\');" onMouseDown="return false;">External</a></span></li>'."\n";
				$contentTabs .= '</ul>'."\n";
			...
			function ltyc_cpt_toggle() {
				...
				$cptSelect .= '<p class="entryTypeToggle">Choose Custom Post Type: <select name="cptToggle" id="cptToggle" size="1">'."\n";
				$post_types = $wpdb->get_col($wpdb->prepare("SELECT DISTINCT(post_type) FROM ".$wpdb->prefix."posts WHERE post_type != 'post' AND post_type != 'page' AND post_type != 'revision' AND post_type != 'attachment' AND post_type != 'nav_menu_item' AND post_status = 'publish'"));
								
				foreach($post_types as $post_type) {					
					if($loopCounter == 0 && !isset($this->customPostType)) { $this->customPostType = $post_type; }
					$cptObj = get_post_type_object($post_type);
					if(!is_object($cptObj)) { $cptObj->labels->name = ucfirst($post_type); }
			...
			function ltyc_tax_toggle() {
				...
				$taxSelect .= '<p class="entryTypeToggle">Choose Taxonomy: <select name="taxToggle" id="taxToggle" size="1">'."\n";
				$taxonomies = $wpdb->get_col($wpdb->prepare("SELECT DISTINCT(taxonomy) FROM ".$wpdb->prefix."term_taxonomy WHERE taxonomy != 'link_category'"));
				
				foreach($taxonomies as $taxonomy) {					
					if($loopCounter == 0 && !isset($this->taxonomy)) { $this->taxonomy = $taxonomy; }
					$taxObj = get_taxonomy($taxonomy);
					if(!is_object($taxObj)) { $taxObj->labels->name = ucfirst($taxonomy); }
			...
			function ltyc_page_heirarchy_output($entryId, $type) {
				...
				if($wpdb->num_rows > 0) {
					foreach($subEntryResults as $subEntry) {	
						...
						$documentSubList .= '<td width="30"><a href="'.$subEntryPermalink.'" title="'.$subEntryTitle.'" class="selectDocument">Select</a></td>'."\n";
						$documentSubList .= '</tr>'."\n";
			...
			function ltyc_tax_ancestors($term_id, &$currentLevel = 0) {
				global $wpdb;
				$ancestorCheck = $wpdb->prepare('SELECT parent FROM '.$this->queryBlog.'term_taxonomy WHERE term_id = '.$term_id.'');
			...
			function ltyc_pagination($type) {
				...
				if($type == 'posts') {
					...
					if(post_type_exists($this->customPostType) === true) {
						...
						$paginationTitle = 'Select '.$cpt_obj->labels->singular_name.':';
						$cpt_hierarchical = $cpt_obj->hierarchical;
						$cpt_page_link = '&cpt='.$this->customPostType;
					}
					else {
						$paginationTitle = 'Select '.ucfirst($this->customPostType).':';
					...
					if(taxonomy_exists($this->taxonomy) === true) {
						...
						$paginationTitle = 'Select '.$tax_obj->labels->singular_name.':';	
						$tax_hierarchical = $tax_obj->hierarchical;
						$tax_page_link = '&tax='.$this->taxonomy;
					}
					else {
						$paginationTitle = 'Select '.ucfirst($this->taxonomy).':';
						$tax_hierarchical = $tax_obj->hierarchical;
					...
					if($type == 'docs' || $type == 'images') {
						if(is_array($entryResults)) {				
							foreach($entryResults as $entry) {
								...
								$documentList .= '<td width="430">'.$entry['post_title'].'</td>'."\n";
								$documentList .= '<td width="50">('.$attachmentType.')</td>'."\n";
								$permalink = $entry['guid'];
								$documentList .= '<td width="30"><a href="'.$permalink.'" title="'.$entry['post_title'].'" class="selectDocument">Select</a></td>'."\n";
								$documentList .= '</tr>'."\n";
							}
						}
					}
						...
						if(is_array($entryResults)) {
							foreach($entryResults as $entry) {
								$documentList .= '<tr>'."\n";
								$documentList .= '<td width="480">'.$entry['post_title'].'</td>'."\n";	
								$documentList .= '<td width="30"><a href="'.get_permalink($entry['ID']).'" title="'.$entry['post_title'].'" class="selectDocument">Select</a></td>'."\n";
								$documentList .= '</tr>'."\n";
						...
						if((isset($tax_obj->hierarchical) && $tax_obj->hierarchical == true) || (isset($tax_hierarchical) && $tax_hierarchical == true)) {
							if(is_array($entryResults)) {
								foreach($entryResults as $entry) {
									$documentList .= '<tr>'."\n";
									$documentList .= '<td width="480">'.$entry['name'].'</td>'."\n";	
									if(taxonomy_exists($this->taxonomy) === true) {
										$documentList .= '<td width="30"><a href="'.get_term_link($entry['slug'], $this->taxonomy).'" title="'.$entry['name'].'" class="selectDocument">Select</a></td>'."\n";
									}
									else {
										if(function_exists('switch_to_blog') && switch_to_blog($this->currentBlog)) {
											$documentList .= '<td width="30"><a href="'.get_bloginfo('url').'/?'.$this->taxonomy.'='.$entry['slug'].'" title="'.$entry['name'].'" class="selectDocument">Select</a></td>'."\n";
											restore_current_blog();
										}
										else {
											$documentList .= '<td width="30"><a href="'.get_bloginfo('url').'/?'.$this->taxonomy.'='.$entry['slug'].'" title="'.$entry['name'].'" class="selectDocument">Select</a></td>'."\n";
		...
		if(isset($linkTYC)) {
			...
			add_action('init', array(&$linkTYC, 'ltyc_addbuttons'));
			// Add the settings link to the plugin admin page
