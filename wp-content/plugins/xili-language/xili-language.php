...
define('XILILANGUAGE_VER','2.5.0'); /* used in admin UI*/
...
class xili_language {
	 ...
	 */	
	function admin_init () {
		// test style file in theme subfolder
				...
				echo '<tr'.$tr_class.'><th>'.$checkline.'</th><td>&nbsp;</td><td>'
					...
					if ( $defaultlanguage != '' &&  $defaultlanguage == $language->slug ) {	
						...
						$the_class = ' class="editing lang-'.$defaultlanguage.'"';
							...
							$the_message = __('select one of these languages !', 'xili-language');
							$the_class = $tr_class;
					}
	...
	*/
	function more_infos_in_plugin_list( $links, $file ) {
		...
		if ( $file == $base ) {
			...
			$links[] = __('Informations and Getting started:', 'xili-language') . ' <a href="http://wiki.xiligroup.org">' . __('Xili Wiki', 'xili-language') . '</a>';
	...
	function more_plugin_row ( $plugin_file, $plugin_data, $status ) {
		...
		if ( $plugin_file == $base ) {
			...
			$cb_col = '<img src="'.plugins_url( 'images/xililang-logo.gif', __FILE__ ).'" alt="xili-language trilogy"/>';
	...
	function languages_settings() { 
		...
		<div id="xili-language-settings" class="wrap columns-2" style="min-width:850px">
	...
	function languages_frontend_settings() { 
		...
		?>
		<div id="xili-language-frontsettings" class="wrap columns-2" style="min-width:840px">
			<?php screen_icon('options-general'); ?>
			<h2><?php _e('Languages','xili-language') ?></h2>
			<h3 class="nav-tab-wrapper">
			<?php $this->set_tabs_line() ?>
	...
	function languages_expert() { 
		...
		?>
		<div id="xili-language-support" class="wrap columns-2" style="min-width:840px">
			<?php screen_icon('options-general'); ?>
			<h2><?php _e('Languages','xili-language') ?></h2>
			<h3 class="nav-tab-wrapper">
			<?php $this->set_tabs_line() ?>
	 ...
	 */
	 function add_help_text( $contextual_help, $screen_id, $screen ) { 
	      ...
	      '<p>' . __('<a href="http://wiki.xiligroup.org/" target="_blank">Xili Wiki Documentation</a>','xili-language') . '</p>' .
